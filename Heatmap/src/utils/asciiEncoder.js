...
function decode(msg) {
  return msg.replace(/[\u2400-\u241F\u2421]/g, (m) => String.fromCodePoint(m === '␡' ? 127 : m.charCodeAt(0) - 9216))
}
function encodeWithHtml(msg, addBR = true) {
  return msg.replace(/[\x00-\x1F\x7F\u2400-\u241F\u2421]/g, (m) => {
    ...
    if (addBR && c === 10) return `<x class="LF">␊</x><br>`
    return `<x class="${classMap[c] || ''}">${substitute(c)}</x>`
  })
}

export { decode, encode, encodeWithHtml, substitute }
